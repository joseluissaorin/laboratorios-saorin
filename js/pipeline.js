...
uniform sampler3D uLut; uniform int uLutN; uniform int uLutOn;
uniform float uGain;
...
vec3 lut3(vec3 c){
  vec3 p = clamp(c, 0.0, 1.0) * float(uLutN - 1);
  vec3 b = floor(p); vec3 f = p - b;
  ivec3 i0 = ivec3(b); int mx = uLutN - 1;
  vec3 c000 = texelFetch(uLut, i0, 0).rgb;
  vec3 c100 = texelFetch(uLut, clamp(i0+ivec3(1,0,0), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c010 = texelFetch(uLut, clamp(i0+ivec3(0,1,0), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c110 = texelFetch(uLut, clamp(i0+ivec3(1,1,0), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c001 = texelFetch(uLut, clamp(i0+ivec3(0,0,1), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c101 = texelFetch(uLut, clamp(i0+ivec3(1,0,1), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c011 = texelFetch(uLut, clamp(i0+ivec3(0,1,1), ivec3(0), ivec3(mx)), 0).rgb;
  vec3 c111 = texelFetch(uLut, clamp(i0+ivec3(1,1,1), ivec3(0), ivec3(mx)), 0).rgb;
...
void main(){
  ...
  outRaw = vec4(raw, 1.0);
  outGraded = vec4(uLutOn == 1 ? clamp(lut3(raw), 0.0, 1.0) : raw, 1.0);
}`;
...
export class Pipeline {
  constructor(canvas, lutData, lutN){
    ...
    this.lutN = lutN;
    this.tLut = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_3D, this.tLut);
  ...
  render(P, time, seed){
    ...
    bindT(3, this.tVideo, u.uVideo); bindT(4, this.tLut, u.uLut, true);
    gl.uniform1i(u.uSrcMode, this.srcMode);
    gl.uniform1f(u.uYNorm, P.yuvNorm);
    gl.uniform1i(u.uFullRange, P.fullRange ? 1 : 0);
    gl.uniform1i(u.uLutN, this.lutN);
    gl.uniform1i(u.uLutOn, P.lutOn ? 1 : 0);
